...
class ConstructorResolver {
	 ...
	 *
	 * Root bean: class [com.shadow.test.UserService]; scope=singleton; abstract=false; lazyInit=null;
	 * autowireMode=0; dependencyCheck=0; autowireCandidate=true; primary=false; factoryBeanName=null;
	 * factoryMethodName=null; initMethodName=null; destroyMethodName=null;
	 * defined in file [D:\netty\spring-framework-master\shadow\out\production\classes\com\shadow\test\UserService.class]
	 * chosenCtors:候选的构造方法
	 * explicitArgs：用户传入的参数
	 */
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		Constructor<?> constructorToUse = null;//使用的构造方法
		ArgumentsHolder argsHolderToUse = null;
		Object[] argsToUse = null; //记录用户是否传入参数

		//如果构造参数不为空就直接使用 这个地方是判断用户传入进来的
			...
			synchronized (mbd.constructorArgumentLock) {
				...
				if (constructorToUse != null && mbd.constructorArgumentsResolved) {
					...
					if (argsToUse == null) {
						...
						//---用于缓存部分准备的构造函数参数的包可见字段） 如果用户没有传入参数 那么就用配置中参数
		...
		if (constructorToUse == null || argsToUse == null) {
			...
			if (candidates.length == 1 && explicitArgs == null && !mbd.hasConstructorArgumentValues()) {
				...
				if (uniqueCandidate.getParameterCount() == 0) {//判断该构造方法的参数是否为空
					synchronized (mbd.constructorArgumentLock) {
						//记录到mbd的缓存中去
						mbd.resolvedConstructorOrFactoryMethod = uniqueCandidate; //记录解析过后的构造方法
						mbd.constructorArgumentsResolved = true;//记录该bd已经进行解析了
						mbd.resolvedConstructorArguments = EMPTY_ARGS;//记录构造参数
					}
					//初始化bean
			...
			Set<Constructor<?>> ambiguousConstructors = null;//存放模糊不清的构造方法
			LinkedList<UnsatisfiedDependencyException> causes = null;//异常列表
            //遍历排序后的构造器
            ...
            // 所以在使用的参数的个数已经大于当前构造方法的参数个数的时候，实际上已经取到了想要的构造方法。
				if (constructorToUse != null && argsToUse != null && argsToUse.length > parameterCount) {
					...
					// Already found greedy constructor that can be satisfied ->
					               // do not look any further, there are only less greedy constructors left.
					                // 如果已经找到选用的构造参数或者需要的参数个数小于当前构造参数个数则终止
					               // 因为已经按照参数个数降序排列
					break;
				}
